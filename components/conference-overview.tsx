...
export function ConferenceOverview({papers,selectedYear,sourcesOk,errors,isLive}:{papers:ResearchPaper[];selectedYear:string;sourcesOk:string[];errors:string[];isLive:boolean}){
  ...
  return <section className="conference-overview" aria-labelledby="conference-overview-title">
    <h2 id="conference-overview-title">{currentYear+1} 届优先 · 下一届投稿日历</h2>
    <p className="overview-note">以北京时间当前年份 {currentYear} 年为基准，关注下一届投稿，不随论文年份筛选改变。双年会议显示下一次举办届次；已截止的下一届仍保留提醒，不意味着还能投稿。</p>
    <Table>
      <TableCaption>全部时间为北京时间（UTC+8）。日期为 {facts.checkedAt} 核验快照；未收录的日期标为待核实，不以往年时间推算，请以官方最新公告为准。</TableCaption>
      <TableHeader><TableRow><TableHead>会议</TableHead><TableHead>目标届次</TableHead><TableHead>投稿截止 · 北京时间</TableHead></TableRow></TableHeader>
      <TableBody>{venues.conference.map(venue=>{
        const target=nextEdition(venue,currentYear);
        return <TableRow key={venue}><TableHead scope="row">{venue}</TableHead><TableCell>{target} 届{target!==currentYear+1?'（双年会议）':''}</TableCell><TableCell><DeadlineCell edition={editions.find(e=>e.venue===venue&&e.year===target)} year={target}/></TableCell></TableRow>;
      })}</TableBody>
    </Table>
    <h2 className="mt-8">{year} 年 · 历史会议占比</h2>
    ...
    <Table>
      <TableCaption>录用总数为 {facts.checkedAt} 人工核验快照，非自动核验。候选数随每日论文库更新；历史截稿日期仅供回顾，不是当前投稿目标。</TableCaption>
      <TableHeader><TableRow><TableHead>会议</TableHead><TableHead>本站渲染候选</TableHead><TableHead>主会录用总数</TableHead><TableHead>本站候选占比</TableHead><TableHead>历史截稿参考</TableHead></TableRow></TableHeader>
      <TableBody>{venues.conference.map(venue=>{
        ...
        return <TableRow key={venue}>
          ...
          <TableCell><details><summary>查看 {year} 届</summary><DeadlineCell edition={edition} year={year} held={held}/></details></TableCell>
        </TableRow>;
      })}</TableBody>
    </Table>
  </section>;

...
export async function POST(request:Request){
 ...
 if(!Array.isArray(data.papers)||!data.papers.length||data.papers.some((p:any)=>!p||typeof p.title!=='string'||typeof p.category!=='string'||typeof p.first_seen!=='string'||![...venues.conference,...venues.journal].includes(p.venue)||!/^https:\/\/(openaccess\.thecvf\.com|www\.ecva\.net|dblp\.org|doi\.org|papers\.nips\.cc|proceedings\.neurips\.cc|proceedings\.mlr\.press|openreview\.net)\//.test(p.url)||!Number.isInteger(p.year)))return new Response('Invalid paper records',{status:400});
